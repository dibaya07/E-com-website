...
import { DisplayCartProp, Products } from "@/app/types/product.types";
...
export default function DisplayCartItems({handleQuantity , handleRemove, handleRouting, cartItems, allCarts } : DisplayCartProp) {
  return (
     <div className="right  w-[70%] ">
              <div >
                ...
                <ul className="my-2 w-full ">
                  {cartItems?.map((item: Products) => {
                    const itemStock = allCarts?.filter(data=>data.id == item._id)[0]
                    return (
                        ...
                        <div className="flex w-full justify-between items-center">
                          ...
                          <span className="flex justify-center items-center gap-4 border border-gray-500/80 h-fit rounded-md">
                              <button className={`text-xs hover:bg-blue-300/40 p-3 rounded-l-md cursor-pointer disabled:text-black/50 disabled:cursor-not-allowed disabled:hover:bg-blue-300/10`} onClick={()=>handleQuantity(item._id,item.stock,"decrease")} disabled={itemStock?.qty <= 1}>
                               ...
                               {item.stock && allCarts.filter(data => data.id == item._id)[0]?.qty}
              ...
              <button className="flex justify-center items-center gap-2 text-blue-600" onClick={handleRouting}> <span className="text-xs"><FaArrowLeftLong /></span><span className="text-sm">Continue Shopping</span></button>
            </div>
  )
}
